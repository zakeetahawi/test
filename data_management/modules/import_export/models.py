"""
نماذج وحدة استيراد وتصدير البيانات
"""
...
class ImportExportLog(models.Model):
    """
    نموذج لتسجيل عمليات الاستيراد والتصدير
    """
    OPERATION_TYPES = [
        ('import', _('استيراد')),
        ('export', _('تصدير')),
    ...
    records_count = models.IntegerField(_('عدد السجلات'), default=0)
    success_count = models.IntegerField(_('عدد السجلات الناجحة'), default=0)
    error_count = models.IntegerField(_('عدد السجلات الفاشلة'), default=0)
    error_details = models.TextField(_('تفاصيل الأخطاء'), blank=True)
    
    # معلومات المستخدم والوقت
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='import_export_logs', verbose_name=_('تم الإنشاء بواسطة'))
